...
class CStencilHardPart : public ILinearOperator<ValueType>
{
 private:
 ...
 public:
    CStencilHardPart(
      ...
      inline static const std::string IDENTIFER = "stencil";
    void apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const;
    ~CStencilHardPart();
};

template <typename ValueType, typename VecType>
CStencilHardPart<ValueType, VecType>::CStencilHardPart(
   const std::size_t p_objCols,
   const std::size_t p_objRows,
   const std::size_t p_objLevels,
   const ValueType p_c,
...
template <typename ValueType, typename VecType>
void CStencilHardPart<ValueType, VecType>::apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const
{
   std::size_t l_thread_id = omp_get_thread_num();
   std::size_t l_nthreads = omp_get_num_threads();
   std::size_t l_i_ltb = m_objLevels * l_thread_id       / l_nthreads;
   std::size_t l_i_utb = m_objLevels * (l_thread_id + 1) / l_nthreads;
   ...
   for (std::size_t l_pos_L=l_i_ltb; l_pos_L<l_i_utb; ++l_pos_L)
   {
