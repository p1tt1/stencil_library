...
class C_Sol_CG_01_Buffer
{
    public:
        inline static const std::string IDENTIFER = "cg_01_buffer";
        std::size_t operator()(
            const std::size_t p_size,
            const IOperator<ValueType> & p_A,
            ...
            const std::size_t p_sSize2d
        ) const;
};

template <typename ValueType>
std::size_t C_Sol_CG_01_Buffer<ValueType>::operator()(
    const std::size_t p_size,
    const IOperator<ValueType> & p_A,
    const ValueType * __restrict__ p_x_0,
    const ValueType * __restrict__ p_b,
...
) const
{
    ...
    ValueType * l_p_raw = new ValueType[p_size+2*p_sSize2d];
    ValueType * l_p = &(l_p_raw[p_sSize2d]);
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp for
        for (std::size_t i = 0; i < p_sSize2d; ++i)
        {
            l_p_raw[i] = 0;
            l_p_raw[p_size+p_sSize2d+i] = 0;
        }
        // --------------------------------------------------------------------
