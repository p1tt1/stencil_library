...
#
> $OUTPUT_FILE
output "node_id,bench_id,impl_id,region_id,obj_cols,obj_rows,obj_levels,threads,iter_max,epsilon,iter,runtime,efficiency"
...
for INDEX in ${!THREAD_LIST[@]}; do
  ...
  CURRENT_CMD="$APP_PATH $OBJ_COLS $OBJ_ROWS $OBJ_LEVELS $APPLY_RUNS"
  ...
  EPSILON=$(echo "$CURRENT_OUTPUT"    | grep "EPSILON_IMPL"                | cut -d ',' -f 2)
  ...
  RUNTIME_OVERALL=$(echo "$CURRENT_OUTPUT"    | grep "RUNTIME_OVERALL_IMPL"                | cut -d ',' -f 2)

  if [ $THREADS -eq $THREADS_MIN ]; then
    RUNTIME_REF_APPLY=$RUNTIME_APPLY
    RUNTIME_REF_OVERALL=$RUNTIME_OVERALL
  fi

  EFFICIENCY_APPLY=$(python3 -c "print($RUNTIME_REF_APPLY/($THREADS*$RUNTIME_APPLY))")
  EFFICIENCY_OVERALL=$(python3 -c "print($RUNTIME_REF_OVERALL/($THREADS*$RUNTIME_OVERALL))")

  output "$NODE_ID,$SCRIPT_ID,$IMPL_ID,$REGION_ID_APPLY,$OBJ_COLS_RESULT,$OBJ_ROWS_RESULT,$OBJ_LEVELS_RESULT,$THREADS,$ITER_MAX,$EPSILON,$ITER,$RUNTIME_APPLY,$EFFICIENCY_APPLY"
  output "$NODE_ID,$SCRIPT_ID,$IMPL_ID,$REGION_ID_OVERALL,$OBJ_COLS_RESULT,$OBJ_ROWS_RESULT,$OBJ_LEVELS_RESULT,$THREADS,$ITER_MAX,$EPSILON,$ITER,$RUNTIME_OVERALL,$EFFICIENCY_OVERALL"
  ...
  log_everywhere_silent "=> Efficiency routine: $EFFICIENCY_OVERALL"
  log_timestap_everywhere_silent
  log_everywhere_silent "------------------"
