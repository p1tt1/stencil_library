...
APP_PATH=${1:-./e_linear_stencil_para}
...
bench_cmd_likwid_perfctr(){
  echo "likwid-perfctr -C $(thread_config $1) -f -g FLOPS_DP -O ${APP_PATH} $OBJ_FACTOR"
}

bench_cmd_likwid_pin(){
  echo "likwid-pin -C $(thread_config $1) ${APP_PATH} $OBJ_FACTOR"
}

# bench_cmd_impl(){
#   # export OMP_NUM_THREADS=$1
#   # export OMP_PROC_BIND=FALSE
#   echo "export OMP_NUM_THREADS=$1 && export OMP_PROC_BIND=FALSE && ${APP_PATH} $OBJ_FACTOR"
# }
bench_cmd_impl(){
  echo "export OMP_NUM_THREADS=$1" > ./env_openmp
  echo "export OMP_PROC_BIND=FALSE" >> ./env_openmp
  # cat ./env_openmp
  source ./env_openmp
  echo "source ./env_openmp && ${APP_PATH} $OBJ_FACTOR"
}
...
for INDEX in ${!THREAD_LIST[@]}; do
  ...
  # CURRENT_CMD=$(bench_cmd_likwid $THREADS)
  # CURRENT_CMD=$(bench_cmd_likwid_pin $THREADS)
  CURRENT_CMD="$(bench_cmd_impl $THREADS)"
  # CURRENT_CMD="export OMP_NUM_THREADS=$THREADS && ${APP_PATH} $OBJ_FACTOR"

  log_everywhere_silent "($(( $INDEX + 1 ))/${#THREAD_LIST[@]}) \$ $CURRENT_CMD"

  # CURRENT_OUTPUT=$($CURRENT_CMD)
  # eval "$CURRENT_CMD"
  CURRENT_OUTPUT=$(export OMP_NUM_THREADS=$THREADS && ${APP_PATH} $OBJ_FACTOR)
