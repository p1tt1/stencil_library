...
for OBJ_SIZE in ${OBJ_SIZES[@]}; do
  for REGION_ID in ${REGION_IDS[@]}; do
    ...
    L3_ACCESS_RATE=$(echo "$OUTPUT_L3"         | grep "L3 access rate \[%\] STAT"             | cut -d ',' -f 2)
    L3_MISS_RATE=$(echo "$OUTPUT_L3"           | grep "L3 miss rate \[%\] STAT"               | cut -d ',' -f 2)
    L3_MISS_RATIO=$(echo "$OUTPUT_L3"          | grep "L3 miss ratio \[%\] STAT"              | cut -d ',' -f 2)
