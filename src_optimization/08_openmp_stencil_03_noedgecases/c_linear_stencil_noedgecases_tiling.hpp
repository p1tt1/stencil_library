...
class CLinearStencilNoedgecasesTiling : public ILinearOperator<ValueType>
{
 private:
 ...
 public:
    CLinearStencilNoedgecasesTiling(
      ...
      inline static const std::string IDENTIFER = "linear_stencil_noedgecases_tiling";
    void apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const;
    ~CLinearStencilNoedgecasesTiling();
};

template <typename ValueType, typename VecType>
CLinearStencilNoedgecasesTiling<ValueType, VecType>::CLinearStencilNoedgecasesTiling(
   const std::size_t p_objCols,
   const std::size_t p_objRows,
   const std::size_t p_objLevels,
   const std::size_t p_objCols_tiling,
...
template <typename ValueType, typename VecType>
void CLinearStencilNoedgecasesTiling<ValueType, VecType>::apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const
{
   ...
   #pragma omp parallel
   {
      ...
      #pragma omp for
      for (std::size_t l_pos_LT0=0; l_pos_LT0<m_objLevels; l_pos_LT0+=m_objLevels_tiling)
      {
         for (std::size_t l_pos_RT0=0; l_pos_RT0<m_objRows; l_pos_RT0+=m_objRows_tiling)
         {
            for (std::size_t l_pos_CT0=0; l_pos_CT0<m_objCols; l_pos_CT0+=m_objCols_tiling)
            {
               for (std::size_t l_pos_LT1=0; l_pos_LT1<m_objLevels_tiling; ++l_pos_LT1)
               {
                  for (std::size_t l_pos_RT1=0; l_pos_RT1<m_objRows_tiling; ++l_pos_RT1)
                  {
                     for (std::size_t l_pos_CT1=0; l_pos_CT1<m_objCols_tiling; l_pos_CT1+=VecType::size())
                     {
                        l_pos_L = l_pos_LT0 + l_pos_LT1;
                        l_pos_R = l_pos_RT0 + l_pos_RT1;
                        l_pos_C = l_pos_CT0 + l_pos_CT1;
