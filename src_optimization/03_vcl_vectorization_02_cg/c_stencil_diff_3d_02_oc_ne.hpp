...
class CStencilDiff3D_02_Oc_Ne : public IOperator<ValueType>
{
 private:
 ...
 public:
   ...
   inline static const std::string IDENTIFER = "02_oc_ne";
   void apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const;
   ~CStencilDiff3D_02_Oc_Ne();
};

template <typename ValueType>
CStencilDiff3D_02_Oc_Ne<ValueType>::CStencilDiff3D_02_Oc_Ne(
   const std::size_t p_sCols,
   const std::size_t p_sRows,
   const std::size_t p_sLevels,
   const std::size_t p_sCols_tiling,
...
template <typename ValueType>
void CStencilDiff3D_02_Oc_Ne<ValueType>::apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const
{
      ...
      for (             std::size_t lI_yT=0; lI_yT<m_sRows;   lI_yT+=m_sRows_tiling)
      {
         for (          std::size_t lI_xT=0;        lI_xT<m_sCols;                lI_xT+=m_sCols_tiling)
         {
            for (       std::size_t lI_z=0;         lI_z<m_sLevels_tiling; ++lI_z)
            {
               ...
               {
                  #pragma omp simd
                  for ( std::size_t lI_x=0;         lI_x<m_sCols_tiling;            ++lI_x)
                  {
