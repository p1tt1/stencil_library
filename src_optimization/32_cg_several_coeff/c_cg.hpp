...
class C_CG: public ISolver<ValueType>
{
    public:
        inline static const std::string IDENTIFER = "cg";
        std::size_t operator()(
...
template <typename ValueType>
std::size_t C_CG<ValueType>::operator()(
    const std::size_t p_size,
    const ILinearOperator<ValueType> & p_A,
    const ValueType * __restrict__ p_x_0,
    const ValueType * __restrict__ p_b,
...
) const
{
    ...
    #pragma omp parallel
    {
        ...
        while(l_iter_t < p_iterMax)
        {
            ...
            if(l_alpha_0_t < p_epsilon * l_alpha_init_t)
            ...
            #pragma omp master
            {
                ...
                // std::cout << "l_alpha_init_t: " << l_alpha_init_t << std::endl;
                // std::cout << "TOL: " << p_epsilon * l_alpha_init_t << std::endl;
