...
template <typename ValueType>
CLinearStencil<ValueType>::CLinearStencil(
   const std::size_t p_objCols,
   const std::size_t p_objRows,
   const std::size_t p_objLevels,
   const ValueType p_c,
...
template <typename ValueType>
void CLinearStencil<ValueType>::apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const
{
   //---------------------------------------------------------------
   // Notes: corners
   //---------------------------------------------------------------
   ...
   for (std::size_t i=m_objSize2d; i<m_objSize3d-2*m_objSize2d+1; i+=m_objSize2d)
   {
      for (std::size_t j=m_objSize1d; j<m_objSize2d-(2*m_objSize1d)+1; j+=m_objSize1d)
      {
         for (std::size_t k=1; k<m_objSize1d-1; ++k)
         {
            p_y[i+j+k] = (1+6*m_factor) * p_x[i+j+k]
                         -    m_factor  * p_x[i+j+k-m_objSize2d]
                         -    m_factor  * p_x[i+j+k-m_objSize1d]
                         -    m_factor  * p_x[i+j+k-1]
                         -    m_factor  * p_x[i+j+k+1]
                         -    m_factor  * p_x[i+j+k+m_objSize1d]
                         -    m_factor  * p_x[i+j+k+m_objSize2d];
         }
