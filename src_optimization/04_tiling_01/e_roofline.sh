...
OBJ_ROWS=1026
OBJ_LEVELS=1026
OBJ_COLS_TILING=1024
OBJ_ROWS_TILING=4
OBJ_LEVELS_TILING=4

THREADS=1

APP_PATH=${1:-./e_linear_stencil_tiling_likwid}
...
for region_id in ${REGION_IDS[@]}; do
  CMD_FLOPS_DP="$(bench_cmd $THREADS FLOPS_DP $region_id) $OBJ_COLS $OBJ_ROWS $OBJ_LEVELS $OBJ_COLS_TILING $OBJ_ROWS_TILING $OBJ_LEVELS_TILING"
  log_everywhere_silent "\$ $CMD_FLOPS_DP"
  OUTPUT_FLOPS_DP=$($CMD_FLOPS_DP)
  # echo "$OUTPUT_FLOPS_DP"
  log_timestap_everywhere_silent

  CMD_MEM="$(bench_cmd $THREADS MEM $region_id) $OBJ_COLS $OBJ_ROWS $OBJ_LEVELS $OBJ_COLS_TILING $OBJ_ROWS_TILING $OBJ_LEVELS_TILING"
  ...
  MFLOP_S=$(echo "$OUTPUT_FLOPS_DP" | grep "DP \[MFLOP/s\]"                | cut -d ',' -f 2)
  MBYTES_S=$(echo "$OUTPUT_MEM"     | grep "Memory bandwidth \[MBytes/s\]" | cut -d ',' -f 2)
