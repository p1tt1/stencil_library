...
APP_PATH=${1:-./e_nonlinear_stencil_mul2}
...
for INDEX in ${!THREAD_LIST[@]}; do
  ...
  FUNC_ID=$(echo "$CURRENT_OUTPUT"         | grep "FUNC_ID_IMPL"         | cut -d ',' -f 2)
  ...
  if [ $THREADS -eq $THREADS_MIN ]; then
    ...
    RUNTIME_REF_ROUTINE=$RUNTIME_OVERALL
  fi

  EFFICIENCY_APPLY=$(python3 -c "print($RUNTIME_REF_APPLY/($THREADS*$RUNTIME_APPLY))")
  EFFICIENCY_ROUTINE=$(python3 -c "print($RUNTIME_REF_ROUTINE/($THREADS*$RUNTIME_OVERALL))")

  output "$NODE_ID,$SCRIPT_ID,$IMPL_ID,$FUNC_ID,$REGION_ID_APPLY,$OBJ_COLS_RESULT,$OBJ_ROWS_RESULT,$OBJ_LEVELS_RESULT,$THREADS,$APPLY_RUNS,$RUNTIME_APPLY,$EFFICIENCY_APPLY"
  output "$NODE_ID,$SCRIPT_ID,$IMPL_ID,$FUNC_ID,$REGION_ID_ROUTINE,$OBJ_COLS_RESULT,$OBJ_ROWS_RESULT,$OBJ_LEVELS_RESULT,$THREADS,$APPLY_RUNS,$RUNTIME_OVERALL,$EFFICIENCY_ROUTINE"
