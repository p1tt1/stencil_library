...
# APP_PATH_MEMORYLESS=./e_nonlinear_stencil_costly_0
# APP_PATH_PRECALC=./e_nonlinear_stencil_precalc_costly_0

# APP_PATH_MEMORYLESS=./e_nonlinear_stencil_costly_1
# APP_PATH_PRECALC=./e_nonlinear_stencil_precalc_costly_1

APP_PATH_MEMORYLESS=./e_nonlinear_stencil_costly_2
APP_PATH_PRECALC=./e_nonlinear_stencil_precalc_costly_2
...
#
> $OUTPUT_FILE
output "node_id,bench_id,impl_id,func_id,region_id,obj_cols,obj_rows,obj_levels,obj_cells,threads,runs,runtime,runtime_avg,efficiency"
...
OBJ_SIZES=($(echo $(seq $OBJ_SIZE_MIN $OBJ_SIZE_STEP $OBJ_SIZE_MAX)))
RUN_ID=0
RUN_MAX=$(( ${#OBJ_SIZES[@]} * 2))
for OBJ_SIZE in ${OBJ_SIZES[@]}; do
  ...
  CMD_PRECALC="$APP_PATH_PRECALC $OBJ_SIZE $OBJ_SIZE $OBJ_SIZE $APPLY_RUNS"
  log_timestap_everywhere_silent
  ...
  RUNTIME_APPLY_PRECALC_AVG=$(python3 -c "print($RUNTIME_APPLY_PRECALC/$APPLY_RUNS)")
  RUNTIME_OVERALL_PRECALC_AVG=$(python3 -c "print($RUNTIME_OVERALL_PRECALC/$APPLY_RUNS)")

  output "$NODE_ID,$SCRIPT_ID,$IMPL_ID_PRECALC,$FUNC_ID_PRECALC,$REGION_ID_APPLY,$OBJ_SIZE,$OBJ_SIZE,$OBJ_SIZE,$OBJ_CELLS,$THREADS,$APPLY_RUNS,$RUNTIME_APPLY_PRECALC,$RUNTIME_APPLY_PRECALC_AVG,$EFFICIENCY_APPLY_PRECALC"
  output "$NODE_ID,$SCRIPT_ID,$IMPL_ID_PRECALC,$FUNC_ID_PRECALC,$REGION_ID_OVERALL,$OBJ_SIZE,$OBJ_SIZE,$OBJ_SIZE,$OBJ_CELLS,$THREADS,$APPLY_RUNS,$RUNTIME_OVERALL_PRECALC,$RUNTIME_OVERALL_PRECALC_AVG,$EFFICIENCY_OVERALL_PRECALC"
  ...
  CMD_MEMORYLESS="$APP_PATH_MEMORYLESS $OBJ_SIZE $OBJ_SIZE $OBJ_SIZE $APPLY_RUNS"
  log_timestap_everywhere_silent
  ...
  RUNTIME_APPLY_MEMORYLESS_AVG=$(python3 -c "print($RUNTIME_APPLY_MEMORYLESS/$APPLY_RUNS)")
  RUNTIME_OVERALL_MEMORYLESS_AVG=$(python3 -c "print($RUNTIME_OVERALL_MEMORYLESS/$APPLY_RUNS)")


  output "$NODE_ID,$SCRIPT_ID,$IMPL_ID_MEMORYLESS,$FUNC_ID_MEMORYLESS,$REGION_ID_APPLY,$OBJ_SIZE,$OBJ_SIZE,$OBJ_SIZE,$OBJ_CELLS,$THREADS,$APPLY_RUNS,$RUNTIME_APPLY_MEMORYLESS,$RUNTIME_APPLY_MEMORYLESS_AVG,$EFFICIENCY_APPLY_MEMORYLESS"
  output "$NODE_ID,$SCRIPT_ID,$IMPL_ID_MEMORYLESS,$FUNC_ID_MEMORYLESS,$REGION_ID_OVERALL,$OBJ_SIZE,$OBJ_SIZE,$OBJ_SIZE,$OBJ_CELLS,$THREADS,$APPLY_RUNS,$RUNTIME_OVERALL_MEMORYLESS,$RUNTIME_OVERALL_MEMORYLESS_AVG,$EFFICIENCY_OVERALL_MEMORYLESS"
