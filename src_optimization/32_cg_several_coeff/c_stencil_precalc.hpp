...
class CStencilPrecalc : public ILinearOperator<ValueType>
{
 private:
 ...
 public:
    CStencilPrecalc(
      ...
      inline static const std::string IDENTIFER = "stencil_precalc";
    void apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const;
    ~CStencilPrecalc();
};

template <typename ValueType, typename VecType>
CStencilPrecalc<ValueType, VecType>::CStencilPrecalc(
   const std::size_t p_objCols,
   const std::size_t p_objRows,
   const std::size_t p_objLevels,
   ValueType * p_c,
...
template <typename ValueType, typename VecType>
void CStencilPrecalc<ValueType, VecType>::apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const
{
   std::size_t l_pos;
