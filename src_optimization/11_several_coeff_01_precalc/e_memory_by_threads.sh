...
APP_PATH=${1:-./e_linear_stencil_precalc_likwid}
...
RUN_ID=0
RUN_MAX=$(( ${#THREAD_LIST[@]} * ${#REGION_IDS[@]} * 3))
for THREADS in ${THREAD_LIST[@]}; do
  for REGION_ID in ${REGION_IDS[@]}; do
    CMD_L2="$(bench_cmd $THREADS L2 $REGION_ID) $APP_PATH $OBJ_COLS $OBJ_ROWS $OBJ_LEVELS"
    log_timestap_everywhere_silent
    log_everywhere_silent "($(( $RUN_ID + 1 ))/$RUN_MAX) \$ $CMD_L2"
    OUTPUT_L2=$($CMD_L2)
    RUN_ID=$(( $RUN_ID + 1 ))

    CMD_L3="$(bench_cmd $THREADS L3 $REGION_ID) $APP_PATH $OBJ_COLS $OBJ_ROWS $OBJ_LEVELS"
    log_timestap_everywhere_silent
    log_everywhere_silent "($(( $RUN_ID + 1 ))/$RUN_MAX) \$ $CMD_L3"
    OUTPUT_L3=$($CMD_L3)
    RUN_ID=$(( $RUN_ID + 1 ))

    CMD_MEM="$(bench_cmd $THREADS MEM $REGION_ID) $APP_PATH $OBJ_COLS $OBJ_ROWS $OBJ_LEVELS"
    log_timestap_everywhere_silent
    ...
    output "$NODE_ID,$SCRIPT_ID,$IMPL_ID,$REGION_ID,$OBJ_COLS,$OBJ_ROWS,$OBJ_LEVELS,$OBJ_CELLS,$THREADS,$L2D_LOAD_BANDWIDTH,$L2D_LOAD_DATA_VOLUME,$L2_BANDWIDTH,$L2_DATA_VOLUME,$L3_ACCESS_BANDWIDTH,$L3_ACCESS_DATA_VOLUMEN,$L3_ACCESS_RATE,$L3_MISS_RATE,$L3_MISS_RATIO,$MEMORY_BANDWIDTH,$MEMORY_DATA_VOLUME"
  done
done
