...
class CStencil : public ILinearOperator<ValueType>
{
 private:
...
template <typename ValueType, typename VecType>
CStencil<ValueType, VecType>::CStencil(
   const std::size_t p_objCols,
   const std::size_t p_objRows,
   const std::size_t p_objLevels,
   const ValueType p_c,
...
template <typename ValueType, typename VecType>
void CStencil<ValueType, VecType>::apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const
{
   std::size_t l_pos;
