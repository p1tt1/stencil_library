...
class CLinearStencilNotiling : public ILinearOperator<ValueType>
{
 private:
 ...
 public:
    CLinearStencilNotiling(
      ...
      inline static const std::string IDENTIFER = "linear_stencil_notiling";
    void apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const;
    ~CLinearStencilNotiling();
};

template <typename ValueType, typename VecType>
CLinearStencilNotiling<ValueType, VecType>::CLinearStencilNotiling(
   const std::size_t p_objCols,
   const std::size_t p_objRows,
   const std::size_t p_objLevels,
   const ValueType p_c,
...
template <typename ValueType, typename VecType>
void CLinearStencilNotiling<ValueType, VecType>::apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const
{
   VecType l_x_LL_Vec;
   VecType l_x_RL_Vec;
   VecType l_x_CL_Vec;
   ...
   //
   lPos = 0;
   p_y[lPos] = (1+3*m_factor) * p_x[lPos]
               -    m_factor  * p_x[lPos+1]
               -    m_factor  * p_x[lPos+m_objSize1d]
               -    m_factor  * p_x[lPos+m_objSize2d];
   ...
   // C5
   //
   lPos = m_objSize3d-m_objSize2d+m_objSize1d-1;
   p_y[lPos] = (1+3*m_factor) * p_x[lPos]
               -    m_factor  * p_x[lPos-m_objSize2d]
               -    m_factor  * p_x[lPos-1]
               -    m_factor  * p_x[lPos+m_objSize1d];
