...
class CNonlinearStencil01 : public INonlinearOperator<ValueType>
{
 private:
 ...
 public:
    CNonlinearStencil01(
      ...
      inline static const std::string IDENTIFER = "nonlinear_01";
...
template <template<typename VecType> typename StateFunc, typename ValueType, typename VecType>
CNonlinearStencil01<StateFunc, ValueType, VecType>::CNonlinearStencil01(
   const std::size_t p_objCols,
   const std::size_t p_objRows,
   const std::size_t p_objLevels,
   ValueType * p_s,
...
template <template<typename VecType> typename StateFunc, typename ValueType, typename VecType>
void CNonlinearStencil01<StateFunc, ValueType, VecType>::setState(const ValueType * __restrict__ p_s)
{
   m_s = p_s;
}

template <template<typename VecType> typename StateFunc, typename ValueType, typename VecType>
void CNonlinearStencil01<StateFunc, ValueType, VecType>::apply(const ValueType * __restrict__ p_x, ValueType * __restrict__ p_y) const
{
   std::size_t l_pos;
   ...
   for (std::size_t l_pos_L=0; l_pos_L<m_objLevels; ++l_pos_L)
   {
      for (std::size_t l_pos_R=0; l_pos_R<m_objRows; ++l_pos_R)
      {
         for (std::size_t l_pos_C=0; l_pos_C<m_objCols; l_pos_C+=VecType::size())
         {
            ...
            // l_stateFunc(l_c_LL_Vec);
            // l_stateFunc(l_c_RL_Vec);
            // l_stateFunc(l_c_CL_Vec);
            // l_stateFunc(l_c_Vec);
            // l_stateFunc(l_c_CU_Vec);
            // l_stateFunc(l_c_RU_Vec);
            // l_stateFunc(l_c_LU_Vec);
